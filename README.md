...
Now the app will be on the URL the command above supplies; typically `https://[PROJECT_ID].ts.r.appspot.com/`, depending on what you called your project.

Finally, your app will not work until you store your app secrets in Google's Secret Manager:

1. Get your oauth_app `client_id` and `client_secret` by setting up an app in ChurchSuite: `ProfileIcon -> Settings -> OAuth Apps`.
...
#### Testing cloud app locally

If you want to test the cloud app locally you need to get a json key file:
...
4. Now you can browse to the app at `localhost:8080`.
